...
int32_t Game::init(const GameConfig& gameCfg) {
    if (_chessBoard.init(gameCfg.chessBoardId, gameCfg.targetId, gameCfg.blinkTargetTimerId) != EXIT_SUCCESS) {
        std::cerr << "_chessBoard.init() in Game::init() failed!" << std::endl;
        return EXIT_FAILURE;
    }

    if (_pieceHandler.init(static_cast<GameBoardProxy*>(&_chessBoard), gameCfg.whitePiecesId, gameCfg.blackPiecesId) != EXIT_SUCCESS) {
