...
SDL_Texture* ImageContainer::getImageTexture(int32_t rsrcId) const {
    ...
    if (it == _textures.end()) {
        std::cerr << "Invalid resourceId in ImageContainer::getImageTexture() " << rsrcId << "requested!" << std::endl;
        return nullptr;
    }
...
const Frames& ImageContainer::getImageFrame(int32_t rsrcId) const {
    ...
    if (it == _textureFrames.end()) {
        std::cerr << "Invalid resourceId in ImageContainer::getImageFrame() " << rsrcId << "requested."
...
int32_t ImageContainer::loadSingleResource(const ImageConfig& imageCfg, int32_t resourceId) {
    SDL_Texture* texture = nullptr;

    if (Texture::createTextureFromFile(imageCfg.location, texture) != EXIT_SUCCESS) {
        std::cerr << "Texture::createTextureFromFile() failed in ImageContainer::loadSingleResource() for file: " 
                  << imageCfg.location << std::endl;
        return EXIT_FAILURE;
    }

    _textures[resourceId] = texture;

    _textureFrames[resourceId] = imageCfg.frames;
